...
public class Log4 {
    ...
    public static class WebsiteMapper extends Mapper<Object, Text, IntWritable, Text> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        	...
        	String re = "(?<website>\\S+)(\\s+)(?<hits>\\S+)";
        	Pattern r = Pattern.compile(re);
            Matcher m = r.matcher(s);
            if (m.find()) {
            	String website = m.group("website");
            	...
            	if (Integer.parseInt(hits) >= Integer.parseInt(conf.get("max"))) {
            		conf.set("max", hits);
                	context.write(new IntWritable(Integer.parseInt(hits)), new Text(website));
            	}
            }
        }
    }
